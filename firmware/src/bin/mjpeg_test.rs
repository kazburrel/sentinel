//! Captures a fixed number of JPEG frames back-to-back (reusing
//! `CameraHandle`, same as `video_test.rs`) and hex-dumps each one to serial
//! immediately after it's captured -- safe because no DMA transfer is
//! in-flight while dumping (the next `capture_jpeg()` call only starts after
//! the current frame's dump finishes), unlike printing *during* a live
//! capture which is known to starve the DMA (see `PROJECT_STATUS.md`,
//! Milestone 4, Blocker #2b).
//!
//! Host-side, `scripts/capture_video.sh` reads the resulting hex dumps and
//! assembles them into an MJPEG video via ffmpeg.
//!
//! Reports the camera's actual capture-only FPS (excluding time spent
//! dumping) separately from total wall time, so the host side can encode the
//! video at a realistic frame rate instead of the much slower
//! capture-and-transmit rate.
